...
### ✅ 최신 완료된 기능들 (2025-09-14)

**구매기간 자동 관리 시스템 구현 완료**:
- **🗃️ 데이터베이스 스키마 확장**: `lotto_draws` 테이블에 `purchase_start_date`, `purchase_end_date` 필드 추가
- **🤖 자동 계산 로직**: 추첨일(토요일) 기준으로 해당 주 일요일~토요일이 구매기간 자동 계산
- **🔄 auto_updater 개선**: 새 회차 업데이트 시 구매기간도 자동으로 계산하여 저장
- **📡 API 확장**: 모든 로또 API에서 구매기간 정보 포함 (`latest`, `draw/{id}`, `draws`)
- **📊 통계 대시보드**: 하드코딩된 구매기간을 API 기반 동적 표시로 변경
- **🚀 프로덕션 배포**: Railway PostgreSQL 마이그레이션 완료 (1185개 회차 일괄 업데이트)
- **🎯 실시간 적용**: 실서버에서 1189회차 구매기간 "09/07 ~ 09/13" 정상 표시
