...
const Recommendation: React.FC = () => {
  ...
  const handleIncludeNumberToggle = (number: number) => {
    setPreferences(prev => {
      const currentIncludes = prev.include_numbers;
      if (currentIncludes.includes(number)) {
        // 이미 포함된 번호면 제거
        return {
          ...prev,
          include_numbers: currentIncludes.filter(n => n !== number)
        };
      } else {
        // 최대 5개까지만 추가 가능
        if (currentIncludes.length >= 5) {
          alert('포함할 번호는 최대 5개까지 가능합니다.');
          return prev;
        }
        // 제외 번호와 중복되지 않는지 확인
        if (prev.exclude_numbers.includes(number)) {
          alert('제외할 번호와 중복됩니다.');
          return prev;
        }
        return {
          ...prev,
          include_numbers: [...currentIncludes, number].sort((a, b) => a - b)
        };
      }
    });
  ...
  const handleExcludeNumberToggle = (number: number) => {
    setPreferences(prev => {
      const currentExcludes = prev.exclude_numbers;
      if (currentExcludes.includes(number)) {
        // 이미 제외된 번호면 제거
        return {
          ...prev,
          exclude_numbers: currentExcludes.filter(n => n !== number)
        };
      } else {
        // 포함 번호와 중복되지 않는지 확인
        if (prev.include_numbers.includes(number)) {
          alert('포함할 번호와 중복됩니다.');
          return prev;
        }
        return {
          ...prev,
          exclude_numbers: [...currentExcludes, number].sort((a, b) => a - b)
        };
      }
    });
  };
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-9 gap-2">
                    {Array.from({ length: 45 }, (_, i) => i + 1).map((number) => (
                      <button
                        key={number}
                        onClick={() => handleIncludeNumberToggle(number)}
                  ...
                  {preferences.include_numbers.length > 0 && (
                    <div className="flex flex-wrap gap-2 items-center">
                      <span className="text-sm text-gray-600">선택된 번호:</span>
                      {preferences.include_numbers.map((num) => (
                        <span key={num} className="inline-flex items-center px-2 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                          {num}
                          <button
                            onClick={() => handleIncludeNumberToggle(num)}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-9 gap-2">
                    {Array.from({ length: 45 }, (_, i) => i + 1).map((number) => (
                      <button
                        key={number}
                        onClick={() => handleExcludeNumberToggle(number)}
                        className={`w-8 h-8 rounded-full text-xs font-medium transition-all ${
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      {preferences.exclude_numbers.map((num) => (
                        <span key={num} className="inline-flex items-center px-2 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800">
                          {num}
                          <button
                            onClick={() => handleExcludeNumberToggle(num)}
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  value={preferences.range_preference}
                  onChange={(e) => handlePreferenceChange('range_preference', e.target.value)}
                >
                  <option value="balanced">균형잡힌</option>
                  <option value="low_heavy">낮은 번호 위주</option>
                  <option value="high_heavy">높은 번호 위주</option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  <option value={0}>연속 번호 없음</option>
